...
export class Tabs extends Component<Props, State> {
	...
	/**
	 * Конструктор класса. Создает начальное состояние из переданных свойств.
	 */
	 ...
	 * @param {number} index - индекс вкладки, по которой щелкнули.
	 */
	 ...
	 * Вычисляет новый индекс активной вкладки, чтобы отбработать случаи, когда закрываются вкладки до актвиной,
	 * активная или единственная. Вызывает внешний обработчик закрытия вкладки, если он был передан.
	 * @param {number} index - индекс вкладки, которую закрыли.
	 */
	...
	/**
	 * Возвращает функцию вывода корешка вкладки. При этом в компонент `Tab` добавляются свойства:
	 ...
	 * @param {boolean} closable - признак того, что вкладки являются закрываемыми
	 */
